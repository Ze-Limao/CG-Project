...
void addPontos(Figura f, Figura toAdd){
    if(f && toAdd){
        ...
        for(unsigned long i = 0; i < getListLength(pontos); i++){
            addValueToList(f->pontos,getListElemAt(pontos, i));
        }
    }
}
...
void figuraToFile(Figura f, const char* path){
    ...
    if (!file){
        printf("Ocorreu um ao abrir o ficheiro '%s'\n", path);
        return;
    }
...
Figura fileToFigura(const char* path){
    ...
    if(figura && file){
        ...
        for(int i = 0; i < vertices; i++){
            ...
            addPonto(figura, newPonto(x, y, z));
        }
        fclose(file);
    }
    return figura;
...
void deleteFigura(void* figura){
    if(figura){
        deepDeleteList(((Figura)figura)->pontos, (void (*)(void *))deletePonto);
        free(figura);
    }
}
